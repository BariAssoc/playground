...
**Version:** v1.0.2 (patch to v1.0.1)
...
**Patch reason:** §10 Versioning & Addendum Rule clarified. Normal versioning is whole increments (v1.0 → v1.2 → v1.3 → ...). Point versions (vX.Y.Z) are emergency-patch only. Each shipped zip is CLOSED — never re-opened or edited in place. v1.0.1 preserved inside this bundle.
...
## §10. Versioning & Addendum Rule 🟢 UPDATED in v1.0.2
...
**Point versions (vX.Y.Z) are emergency-patch only.** Used when a canon-level correction or a missing structural element must be added without a full new content cycle. Not the normal pattern. v1.0.1 was a one-time exception to add the three role briefing READMEs as a standing requirement. v1.0.2 is the canon-clarification patch for this rule itself.
...
**At each version lock, Claude states explicitly:** *"vX.Y is closed. Next upload begins vX.Y+1."* No ambiguity about whether a version is still editable.
...
**Canon + core READMEs (same as v1.0):**
...
**Role-specific briefing READMEs (added in v1.0.1, standing requirement):**
- `README_ZAKIY_vX.md` — developer action briefing, written in Val's voice to Zakiy
- `README_MARKETING_vX.md` — brand/pitch briefing, written in Val's voice for Nikita and marketing team
- `README_RESEARCH_vX.md` — clinical/research briefing, written in Val's voice for Pamela, Professor Ilić, Isaiah
...
## §20. Role-Specific Briefing READMEs 🟢 LOCKED (added in v1.0.1)

Every version bundle from v1.0.1 forward includes three role-specific briefing READMEs in addition to the core HUMAN and CODE READMEs:

- **README_ZAKIY_vX.md** — developer action briefing. Written in Val's voice TO Zakiy. Priorities, open questions, what to build first, component contracts. Sits on top of the CODE README, does not duplicate it.
- **README_MARKETING_vX.md** — brand and pitch briefing. Written in Val's voice FOR Nikita and extended marketing team. Headlines, lift-ready story blocks, emotional beats, brand moves, deck placement suggestions.
- **README_RESEARCH_vX.md** — clinical and research briefing. Written in Val's voice FOR Pamela, Professor Ilić, and Isaiah. Clinical framing per shot, validation gaps, simulation items, open questions, downstream research output.

**Audience and distribution:** all three briefings are written with Val as the recipient and sole distributor. Val personally hands these to team members. They are framed as documents speaking FROM Val, not summaries generated by Claude.

**Display protocol at each version lock:** one role's SHOT-001 draft is displayed in chat for review before building. Because three voices exist, all three SHOT-001 drafts are displayed together in one review pass, unless Val requests otherwise. After approval, remaining shots are built silently across all three briefings in parallel.

**Canonical requirement from v1.0.1 forward:** no version bundle ships without all three role READMEs included, regardless of phase or shot count.

---

*End of CCO-UX-SCREENS-001 v1.0.2*
