...
# UI Pack — Design Decisions (v0.1.0 LOCKED, May 11, 2026)

This section records every architecture decision made when building the `@bariaccess-lite/ui` workspace. Same audit-defensible format as the backend decisions above. Every choice cites either the canon (CCO-LITE-BETA-UI-001) or an explicit founder override in the May 11 build session.

---

## UI Decision 1 — Build tool: Vite + React

**Decided:** Vite 5 + React 18 + TypeScript ESM.

**Alternative considered:** Next.js (SSR-capable, file-based routing).

**Why Vite:** Lite Beta is a mobile-first SPA wrapped by the native iOS shell — no SSR requirement. Vite gives sub-second HMR for Zakiy's iterate loop. Next.js's routing and SSR machinery are overhead with no benefit for this product.

**Authorized:** Founder, this conversation, May 11, 2026 — "My recommendations (left column) match the backend conventions, are production-grade."

---

## UI Decision 2 — State management: Zustand

**Decided:** Zustand 4 with `persist` middleware on `abaStore` and `qStore`.

**Alternative considered:** Redux Toolkit + RTK Query.

**Why Zustand:** Lite Beta has 5 distinct state slices (expression, jotform, aba, parkingLot, q). Each is independently consumable. Zustand's atomicity matches that shape. Redux Toolkit's middleware/store/slice setup pays its complexity tax only when state is deeply cross-cutting — Lite is not.

**Persistence policy:**
- `abaStore` persists `name` and `voiceId` to localStorage (key: `bariaccess.aba.v1`)
- `qStore` persists full inbox (key: `bariaccess.q.v1`)
- `jotformStore` and `parkingLotStore` are session-scoped (data lives in jotformStore + adapter persistence later)
- `expressionStore` is session-scoped (resets on reload — by design, per §2 "expression is current state, not history")

**Authorized:** Founder, this conversation, May 11, 2026.

---

## UI Decision 3 — Data fetching: TanStack Query

**Decided:** TanStack Query 5 for all backend bindings. One `QueryClient` at App root.

**Alternative considered:** Plain `fetch` + custom hooks.

**Why TanStack Query:** Caching, retries, stale-while-revalidate, and background refetch — all out of the box. Crucial for Lite Beta where the user pulls down to refresh, switches between Q ↔ Main ↔ Archive screens, and expects scores to feel "instant" on the second visit.

**Defaults:**
```ts
{ retry: 1, refetchOnWindowFocus: false, staleTime: 60_000 }
```

`refetchOnWindowFocus: false` because mobile foregrounding events fire too aggressively and would saturate the backend in field testing.

**Authorized:** Founder, this conversation, May 11, 2026.

---

## UI Decision 4 — Component primitives: Build custom (no Radix / shadcn)

**Decided:** Every UI primitive (Ollie/ABA icon, ExpressionBubble, PhoneFrame, JotFormPrompt, ReminderPicker, etc.) is hand-written.

**Alternative considered:** Radix UI primitives + custom Tailwind styling.

**Why custom:** Lite Beta's visual identity is non-standard (iOS-style phone frame, dual AI icon stack, 16-segment Bookshelf, color-expressive Ollie's Space). Radix primitives would be styled into invisibility — net cost is positive only when behavior (focus management, modals, dropdowns) is complex. Lite's overlays are mostly bottom sheets and inline pickers, which are simpler than what Radix optimizes for.

**Tradeoff accepted:** No built-in keyboard/focus management. v0.1.1 of UI pack should add Radix `Dialog` for `CardOverlay` and `Popover` for `ReminderPicker` if accessibility audit flags issues.

**Authorized:** Founder, this conversation, May 11, 2026.

---

## UI Decision 5 — Workspace placement: monorepo `packages/ui`

**Decided:** Add as a third npm workspace alongside `shared` and `backend`. Zakiy runs one `npm install` at root and gets the whole stack.

**Alternative considered:** Separate repo (`bariaccess-lite-ui`).

**Why monorepo:** Types in `@bariaccess-lite/shared` (BeaconBand, ScoreResult, CompositeResult) are the contract between backend and UI. Sharing them via npm workspace means a type change in backend breaks UI typecheck immediately — exactly the failure we want at build time, not at runtime.

**Authorized:** Founder, this conversation, May 11, 2026.

---

## UI Decision 6 — Route system: hash-based, no React Router

**Decided:** Simple `window.location.hash` parser. 4 routes: `#sim` (default) / `#main` / `#q` / `#archive`.

**Why no React Router:** Adds 50+ kB to bundle for navigation between 4 sibling screens. The hash approach is 30 lines of code and ships zero new dependencies.

**Tradeoff accepted:** Cannot deep-link to specific JotForm or Memory Snap items. When that's needed, swap to React Router or TanStack Router and use proper URL params.

**Authorized:** Claude (executive coder), May 11, 2026. Founder ratification pending.

---

## UI Decision 7 — Stub adapters for backend-pending endpoints

**Decided:** UI canon defines surfaces (JotForm submission, Q persistence, ABA profile, Memory Snap) that backend v0.1.2 does NOT expose. Rather than block UI build, ship adapters with localStorage implementations and `TODO (backend):` markers.

**File:** `packages/ui/src/api/adapters.ts`.

**Pattern:** Same as backend `NightlyJobAdapters` in v0.1.2 — interface boundary that Zakiy can swap from localStorage to real HTTP without touching consuming code.

**Authorized:** Claude, May 11, 2026. Same architectural pattern as backend v0.1.2 §Decision 23.

---

## UI Decision 8 — Test scope: state machines + canon constants, not full DOM

**Decided:** Tests cover pure logic (Zustand stores, flow orchestrators, palette mappings, constant values) rather than React rendering.

**Why:** State machine and canon-constant tests catch the bugs Zakiy is most likely to introduce (timing constants, ONE CHANCE enforcement, ABA name interpolation, 72h expiry, Daily Pulse ordering). DOM render tests catch class-name regressions, which are visually obvious in dev and cheap to discover.

**Coverage:** ~30 assertions across 7 vitest files. Future v0.1.1 should add `@testing-library/react` smoke tests for `SimulationScreen` and `MainScreen` once flows are stable.

**Authorized:** Claude, May 11, 2026.
