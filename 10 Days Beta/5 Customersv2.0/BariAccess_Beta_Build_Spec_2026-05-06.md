...
**Version:** v3 (May 6, 2026 afternoon — adds Mid-Day Nudges, V-Tag system, expanded cohort, updated timeline)
**To:** Zakiy
...
**Beta cohort:** 11 people across 6 archetypes

---

## 1. Scope — What We're Building

A working beta of the Routine Bookshelf with timer-triggered FABs, three mid-day text nudges, and morning/evening JotForms for an 11-person cohort. Data collection runs May 7–May 17. Goal at Day 11: demonstrate V1/V2/V3/V4 score generation from real human routine + nudge + JotForm data.
...
## 2. The 11-Person Cohort (Updated)
...
| # | Item | Status |
|---|---|---|
| 1 | Grit Engine formula (multiplier rules, 1.0–2.0x trigger conditions) | Pending |
| 2 | Effort Score formula: E = 0.40·F + 0.30·C + 0.30·LC | Pending |
| 3 | Mood scoring scale (input format, range, how it feeds V2) | Pending |
| 4 | The 110 FABs for the cohort (10 per person × 11 people) | Partial — original 5 done, 6 more needed |
...
- **Locked as Reading B:** card popup remains the primary interaction; Journal is a downstream display
- New CosmosDB container `journal_entries`
- Single write at end of cool-down handler (~5 lines of code)

### 6.4 Mid-Day Text Nudge System (NEW in v3)
...
See BETA-NUDGE-001 for full templates and delivery logic.

**Critical loop closure:** AM JotForm Q5 (productivity prediction) connects to 1 PM Nudge Q1 (on-track check). Same person predicts in the morning, gets checked midday — capturing predicted vs actual productivity behavior.

### 6.5 V1–V4 Sub-JSON V-Tag Architecture (NEW in v3)

Per the May 6 Bookend Digital Signature lock, every captured data point carries a V-tag array. Same content can route to multiple V-streams; context classifier dictates which score it feeds.
...
```json
{
  ...
  "v_tags": ["V2", "V3"],
  "fab_flag": false,
  "context_classifier": "stress | nutrition | sleep | productivity",
  "user_id": "string",
  "timestamp": "ISO 8601"
}
```

See BETA-VTAG-001 for the full V-tag map covering every JotForm question, every nudge, and the wearable streams.

### 6.6 Campaigns Feature (Web App Delivery)

Zakiy's existing campaigns feature in the provider web app handles all auto-firing:

- Select cohort (all 11 members)
- Set 10-day window (May 7 – May 17)
- Schedule JotForms (AM, PM) and nudges (10 AM, 1 PM, 3:30 PM)
- Delivery channel per cohort member (SMS / iMessage / WhatsApp / Email)

No manual sending. Once configured, the campaign runs automatically across the 10-day window.
...
3. Claude generates 30 days of plausible prior routine data per user (FAB completion, mood traces, sleep windows, Journal entries) — all tagged `synthetic: true`
...
This is backend tooling — not user-facing AI. Synthetic data is clearly tagged for later cleanup.
...
**Day 11 special report (May 18):** Zakiy → Val full 30-day-trend update covering all 11 cohort members. This is the deliverable that lets Val plan Belgrade pitch (May 27).
...
Spike API integrated. Cohort members without a wearable run as manual-input-only.

---

## 10. Production Timeline (Updated v3)

| Date | Owner | Milestone |
|---|---|---|
| Wed May 6 (today) | Val + Zakiy | Spec v3 + nudge templates + V-tag map delivered |
| Thu May 7 | Zakiy | Campaign launches — AM JotForm + 3 nudges + PM JotForm fire for all 11 |
| Fri May 8 | Val | Grit/Effort/Mood/V1–V4 deliverables to Zakiy |
| Sat May 9 – Sun May 10 | Zakiy | Routine Bookshelf + Bookend coding |
| Mon May 11 | Val | 90-Day Intake + Day 1 Baseline JotForm sent to all 11 |
...
| Sun May 18 | Zakiy → Val | **30-day-trend update report** |
...
2. **Wearable per cohort member** — confirm what each of the 11 is wearing by May 8
3. **Methodical Achiever archetype FABs** — need 10 FABs each for Grace, Jennifer, Madeline, Pamela, Donna (Val to draft)
...
6. **Time-zone handling** — confirm all 11 in same zone for May 7–17, or stagger nudge times
...
| Document | ID | Purpose |
|---|---|---|
| Cohort FABs | BETA-FAB-COHORT-001 | 110 FABs across 6 archetypes (50 done, 60 pending) |
| Universal Bookend | BETA-BOOKEND-001 | Warm-up + cool-down + Journal log spec |
| Day 1 Baseline JotForm | BETA-JF-BASELINE-001 | Chronotype + 25-Q personality |
| AM Daily JotForm | BETA-JF-AM-001 (v2) | 8 questions including productivity prediction |
| PM Daily JotForm | BETA-JF-PM-001 | 9 questions |
| Mid-Day Nudges | BETA-NUDGE-001 | 10 AM / 1 PM / 3:30 PM templates |
| V-Tag Mapping | BETA-VTAG-001 | Sub-JSON V1–V4 reference for Zakiy |
| 90-Day Profile Intake | BETA-INTAKE-001 | One-time deep intake (sent May 11) |
| Cohort JotForm Packets | BETA-JF-COHORT-001 | Per-person packets for all 11 cohort members |
