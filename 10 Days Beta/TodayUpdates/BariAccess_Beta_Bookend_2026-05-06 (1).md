...
**Version:** v3 (May 6, 2026 — cohort updated to 13 people, 7 archetypes)
**Author:** Valeriu E Andrei MD, President — BariAccess LLC
**Status:** Beta v3 — universal template (no contextual variants)
**Linked spec:** BariAccess Beta Build Spec — 10-Day Production Sprint (May 6, 2026)
**Cohort:** 13 people across 7 archetypes (see Master Cohort FABs v5 for full roster)
**Supersedes:** v2 (5-person cohort)

---

## What's New in v3

- **Cohort updated:** 5 people → **13 people across 7 archetypes**
- **Bookend interaction model unchanged** — same warm-up + cool-down + Journal Log structure as v2
- **Same per-FAB friction** (~10 sec, 4 user clicks) regardless of cohort size
- **Embodied Practitioner archetype** (Lana) uses the same universal Bookend — no archetype-specific variants in this beta
- **Internal-only flag** added to Journal entry schema for Lana

The Bookend interaction itself was correct in v2. This bump only updates the cohort context and adds the `internal_only` field to the schema.
...
```json
{
  ...
  "archetype": "string (one of 7 archetypes — NEW v3)",
  "internal_only": "boolean (NEW v3 — true for Lana only)",
...
```json
{
  ...
  "critical_flag": "boolean",
  "internal_only": "boolean (NEW v3 — true for Lana only; routes around PROFEX dashboards)",
  "archetype": "string (one of 7 archetypes — NEW v3)",
  "synthetic": "boolean"
}
```
...
**Across 10 FABs per day:** ~60 taps, ~100 seconds of friction per cohort member. Sustainable for 10 days.

**Across the full 13-person cohort:** ~130 FABs × 10 days = 1,300 FAB cycles total. Each writes 1 warm-up + 1 cool-down event + 1 Journal entry → 3,900 records minimum across the May 7–17 window (excluding wearable/JotForm/nudge data).
...
6. **NEW v3:** Confirm internal-only routing for Lana — Bookend events with `internal_only: true` should NOT surface in any cohort dashboard visible to PROFEX channel. Aggregate cohort statistics MAY include her data flagged as "internal-beta inclusion."
